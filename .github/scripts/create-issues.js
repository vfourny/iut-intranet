...
const LABELS = [
  ...
  { name: 'type: chore',      color: 'e4e669', description: 'Build, tooling, infrastructure' },
  { name: 'type: docs',       color: '0075ca', description: 'Documentation' },
  { name: 'type: refactor',   color: 'fbca04', description: 'Code refactoring' },
  // Priority
  { name: 'priority: critical', color: 'b60205', description: 'Critical – blocks everything' },
  ...
  // Scope
  { name: 'scope: auth',        color: 'c5def5', description: 'Authentication & authorization' },
  { name: 'scope: directory',   color: 'c5def5', description: 'Annuaire / user profiles' },
  { name: 'scope: news',        color: 'c5def5', description: 'Actualités / articles' },
  { name: 'scope: newsletter',  color: 'c5def5', description: 'Newsletter & email subscriptions' },
  { name: 'scope: agenda',      color: 'c5def5', description: 'Agenda & events' },
  { name: 'scope: hr',          color: 'c5def5', description: 'Espace RH & documents' },
  { name: 'scope: photos',      color: 'c5def5', description: 'Photothèque' },
  { name: 'scope: plans',       color: 'c5def5', description: 'Plans des sites & déplacements' },
  { name: 'scope: org-chart',   color: 'c5def5', description: 'Organigramme' },
  { name: 'scope: ui',          color: 'c5def5', description: 'UI components & design system' },
  { name: 'scope: infra',       color: 'c5def5', description: 'Infrastructure & DevOps' },
  { name: 'scope: db',          color: 'c5def5', description: 'Database & Prisma schema' },
  // Special
  ...
  { name: 'dependencies',       color: '0075ca', description: 'Dependency updates' },
  { name: 'status: blocked',    color: 'e4e669', description: 'Blocked by another issue or external factor' },
  { name: 'status: needs-info', color: 'e4e669', description: 'More information required' },
];

// Get existing labels
const existingLabelsRes = await github.rest.issues.listLabelsForRepo({ owner, repo, per_page: 100 });
...
for (const label of LABELS) {
  ...
  try {
    ...
    console.warn(`  ⚠️  Impossible de créer le label "${label.name}" :`, err.message);
  }
}
console.log('');
...
const issues = [
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    body: taskBody({
      ...
      description: "Initialiser la structure du monorepo : packages `web` (Nuxt), `api` (tRPC server), `db` (Prisma), `email`, `shared-types`, `ui` (composants Vue partagés). Configurer Turborepo, ESLint, Prettier, TypeScript strict, scripts globaux. Mettre en place `.env` centralisé et `globalDependencies` pour invalidation cache.",
      steps: '- [ ] Créer la structure Turborepo avec tous les packages\n- [ ] Configurer ESLint + Prettier partagés\n- [ ] Configurer TypeScript strict dans chaque package\n- [ ] Mettre en place les scripts globaux (dev, build, lint, typecheck)\n- [ ] Configurer `.env` centralisé et `.env.example`\n- [ ] Configurer `globalDependencies` Turborepo pour invalidation cache\n- [ ] Vérifier que `turbo run build` fonctionne',
      additionalContext: 'Point d\'entrée du projet. Toutes les autres issues dépendent de cette base.',
    }),
  },
  {
    ...
    body: taskBody({
      taskType: '⚙️ Backend',
      description: "Initialiser Prisma dans `packages/db`. Configurer la connexion PostgreSQL. Schéma initial avec `User`, `Session`, et les enums `Site`, `Department`, `Specialty`. Système de migrations + seed TypeScript.",
      steps: '- [ ] Initialiser Prisma dans `packages/db`\n- [ ] Configurer `DATABASE_URL` dans `.env`\n- [ ] Créer le schéma initial : `User`, `Session`, enums `Site`, `Department`, `Specialty`\n- [ ] Créer la première migration\n- [ ] Écrire le seed TypeScript de base\n- [ ] Documenter la procédure de migration',
      additionalContext: 'Dépend de #1 (Setup monorepo). PostgreSQL sera provisionné par JRCanDev en prod.',
    }),
  },
  {
    ...
    body: taskBody({
      ...
      description: "Demander et récupérer auprès de l'IUT : fichiers logo principal (couleur, monochrome) en SVG, les 8 logos de département en SVG, valeurs RGB officielles. Intégrer dans `packages/ui/assets/`.",
      steps: "- [ ] Contacter le service communication IUT-LCO\n- [ ] Obtenir logo principal couleur SVG\n- [ ] Obtenir logo principal monochrome SVG\n- [ ] Obtenir les 8 logos de département SVG\n- [ ] Vérifier les valeurs RGB officielles vs conversions CMYK\n- [ ] Intégrer les assets dans `packages/ui/assets/`\n- [ ] Documenter les règles d'usage (zone de protection, interdits)",
      additionalContext: "Couleurs de référence : Rouge IUT #E30613, Bleu IUT #00AEEF, Gris IUT #737373.",
    }),
  },
  {
    ...
    body: taskBody({
      taskType: '🎨 Frontend',
      description: "Définir tous les tokens CSS dans `packages/ui` : couleurs principales IUT, couleurs des 5 spécialités, typographie Arial, espacements. Configurer Tailwind avec ces tokens. Documenter les règles d'usage du logo.",
      steps: "- [ ] Créer le fichier de variables CSS globales dans `packages/ui`\n- [ ] Définir les couleurs principales : `--color-iut-red: #E30613`, `--color-iut-blue: #00AEEF`, `--color-iut-gray: #737373`\n- [ ] Définir les couleurs spécialités (5 valeurs)\n- [ ] Configurer `tailwind.config.ts` avec `theme.extend.colors` pointant vers les tokens\n- [ ] Définir les tokens de typographie (Arial / Arial Narrow)\n- [ ] Documenter les règles logo dans `packages/ui/README.md`",
      additionalContext: 'Dépend de #3 (assets charte). Ces tokens sont utilisés dans tout le projet dès le Sprint 1.',
    }),
  },
  {
    ...
    body: taskBody({
      taskType: '🎨 Frontend',
      description: 'Configurer Nuxt avec TypeScript, Tailwind, PrimeVue. Appliquer le thème PrimeVue conforme à la charte IUT-LCO (couleurs primaires, typo Arial). Layout de base.',
      steps: "- [ ] Configurer Nuxt 3 dans `packages/web` avec TypeScript strict\n- [ ] Intégrer Tailwind CSS avec les tokens IUT-LCO\n- [ ] Intégrer PrimeVue et créer un preset thème IUT-LCO\n- [ ] Configurer Pinia et Pinia Colada\n- [ ] Mettre en place VeeValidate\n- [ ] Créer le layout de base `layouts/default.vue`\n- [ ] Vérifier le rendu sur mobile",
      additionalContext: 'Dépend de #4 (design tokens). Le thème PrimeVue doit utiliser les variables CSS IUT-LCO.',
    }),
  },
  {
    title: '[Chore] Setup tRPC + Inversify + Better Auth',
    milestone: ms1,
    labels: ['type: chore', 'priority: critical', 'scope: auth', 'scope: infra'],
    body: taskBody({
      taskType: '⚙️ Backend',
      description: "Configurer tRPC server avec Inversify pour la DI. Intégrer Better Auth (email/password). Créer les middlewares : `publicProcedure`, `authenticatedProcedure`, `adminProcedure`, `editorProcedure`, `hrProcedure`. Client tRPC côté Nuxt avec Pinia Colada.",
      steps: "- [ ] Configurer tRPC server dans `packages/api`\n- [ ] Mettre en place Inversify (DI) avec les décorateurs TypeScript\n- [ ] Intégrer Better Auth avec provider email/password\n- [ ] Créer `publicProcedure`\n- [ ] Créer `authenticatedProcedure` (vérifie session)\n- [ ] Créer `adminProcedure` (rôle ADMIN)\n- [ ] Créer `editorProcedure` (rôles ADMIN + EDITOR)\n- [ ] Créer `hrProcedure` (rôles ADMIN + HR)\n- [ ] Configurer le client tRPC dans `packages/web` avec Pinia Colada\n- [ ] Tester un appel end-to-end",
      additionalContext: "Les erreurs métier utilisent `AppError` custom propagées via Inversify. Validation Zod obligatoire sur toutes les procédures.",
    }),
  },
  {
    ...
    body: userStoryBody({
      asA: 'développeur',
      iWant: 'avoir des modèles de données cohérents pour les sites, départements et spécialités de l\'IUT-LCO',
      soThat: 'tous les autres modules (annuaire, agenda, actualités…) puissent s\'y référer de manière typée',
      acceptanceCriteria: "- [ ] Enum `Site` : `BOULOGNE`, `CALAIS`, `DUNKERQUE`, `SAINT_OMER`\n- [ ] Enum `Specialty` : 5 valeurs\n- [ ] Enum `Department` : 8 valeurs (GACO, GEA, TC, INFO, GEII, GIM, GB, GTE)\n- [ ] Table de référence `Department` seedée avec métadonnées (site, spécialité, couleur hex, libellé complet)\n- [ ] Types partagés dans `packages/shared-types`\n- [ ] Migration et seed fonctionnels",
      additionalContext: "Couleurs spécialités : Administration/Gestion/Commerce #D71F69, Électricité/Automatique/Informatique #0089CF, Sciences Industrielles #4D5C66, Chimie/Biologie #7FB539, Construction/Énergie #EE7F00",
    }),
  },
  {
    title: '[Feat] Modèle User et système de rôles',
    milestone: ms1,
    labels: ['type: feature', 'priority: critical', 'scope: auth', 'scope: db'],
    body: userStoryBody({
      asA: 'administrateur système',
      iWant: 'disposer d\'un modèle User complet avec rôles et rattachement organisationnel',
      soThat: 'les permissions soient correctement appliquées et l\'annuaire reflète la structure de l\'IUT',
      acceptanceCriteria: "- [ ] Champs : `firstName`, `lastName`, `email`, `phone`, `photoUrl`, `jobTitle`\n- [ ] Champ `departmentId` (FK optionnel vers `Department`)\n- [ ] Champ `service` (texte libre, pour le personnel transverse)\n- [ ] Champ `managerId` (auto-référence optionnelle)\n- [ ] Enum `Role` : `ADMIN`, `EDITOR`, `HR`, `USER`\n- [ ] Contrainte : soit `departmentId` soit `service` renseigné\n- [ ] Migration et seed avec utilisateurs de test",
      additionalContext: "Les utilisateurs sans département (RH, scolarité, direction…) utilisent le champ `service` libre. Le champ `managerId` est nécessaire pour l'organigramme du Sprint 4.",
    }),
  },
  {
  ...
  },
  {
    ...
    body: userStoryBody({
      ...
      iWant: 'avoir un composant Vue réutilisable affichant le badge d\'un département',
      soThat: "l'appartenance organisationnelle soit clairement visible et cohérente dans tout l'intranet",
      acceptanceCriteria: "- [ ] Affiche sigle du département (ex: INFO)\n- [ ] Affiche le site (ex: Calais)\n- [ ] Couleur de fond selon la spécialité\n- [ ] Fallback pour les utilisateurs en service transverse (libellé libre + couleur neutre)\n- [ ] Composant typé TypeScript (props validées)\n- [ ] Tailles variants (sm, md, lg)\n- [ ] Accessible (contraste suffisant)",
      additionalContext: "Utilisé dans l'annuaire, les articles, les événements, la photothèque. Voir #7 pour les couleurs de spécialité.",
    }),
  },
  {
  ...
  },
  {
    ...
    body: taskBody({
      taskType: '🛠️ DevOps / Infra',
      description: 'Configurer GitHub Actions : lint, typecheck, build sur PR. Déploiement preview automatique. Coordination avec JRCanDev pour environnement de prod.',
      steps: "- [ ] Workflow CI : lint + typecheck sur chaque PR\n- [ ] Workflow CI : build sur chaque PR\n- [ ] Workflow CD : déploiement preview automatique sur PR\n- [ ] Badge CI dans le README\n- [ ] Coordination avec JRCanDev pour l'env prod",
      additionalContext: 'Les workflows Turborepo doivent utiliser le cache pour accélérer les builds.',
    }),
  },
  {
    ...
    body: taskBody({
      taskType: '🔍 Research / Spike',
      description: "Recueillir les contraintes d'hébergement auprès de JRCanDev : Docker ? reverse proxy ? PostgreSQL provisionné ? domaine ? certificats SSL ? procédure de déploiement ? Documenter dans le repo.",
      steps: "- [ ] Réunion avec l'équipe JRCanDev\n- [ ] Clarifier : Docker / docker-compose ou autre ?\n- [ ] Clarifier : reverse proxy (Nginx, Caddy, Traefik ?)\n- [ ] Clarifier : PostgreSQL provisionné ou à déployer ?\n- [ ] Clarifier : domaine et sous-domaine\n- [ ] Clarifier : certificats SSL\n- [ ] Documenter la procédure de déploiement dans `docs/deployment.md`",
      additionalContext: "Cette issue bloque potentiellement le déploiement preview et la mise en production finale.",
    }),
  },
  {
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    body: taskBody({
      taskType: '🛠️ DevOps / Infra',
      description: "Créer un bucket Scaleway (région Paris). Package `storage` dans le monorepo avec client S3-compatible. Helpers pour upload, génération d'URLs signées, suppression.",
      steps: "- [ ] Créer le bucket Scaleway (région Paris, hébergement France)\n- [ ] Créer `packages/storage` avec client S3-compatible\n- [ ] Helper `uploadFile(key, buffer, mimeType)`\n- [ ] Helper `getSignedUrl(key, expiresIn)`\n- [ ] Helper `deleteFile(key)`\n- [ ] Configurer les variables d'env : `SCALEWAY_ACCESS_KEY`, `SCALEWAY_SECRET_KEY`, `SCALEWAY_BUCKET`, `SCALEWAY_ENDPOINT`\n- [ ] Mettre à jour `.env.example`\n- [ ] Tester upload d'une image de test",
      additionalContext: "Scaleway Object Storage est S3-compatible. Endpoint : `s3.fr-par.scw.cloud`. Utilisé pour photos de profil, images d'articles, documents RH et photothèque.",
    }),
  },
  {
    title: '[Feat] Upload photo de profil',
    milestone: ms2,
    labels: ['type: feature', 'priority: high', 'scope: photos', 'scope: directory'],
    body: userStoryBody({
      asA: 'utilisateur connecté',
      iWant: 'uploader et recadrer une photo de profil',
      soThat: "mes collègues puissent me reconnaître facilement dans l'annuaire",
      acceptanceCriteria: "- [ ] Composant d'upload avec preview avant envoi\n- [ ] Recadrage côté client (format carré, ratio 1:1)\n- [ ] Validation taille max (ex: 5 Mo)\n- [ ] Validation format (JPEG, PNG, WebP)\n- [ ] Upload vers Scaleway Object Storage\n- [ ] Mise à jour `User.photoUrl` via procédure tRPC\n- [ ] Suppression de l'ancienne photo si remplacement\n- [ ] Feedback de progression d'upload",
      additionalContext: 'Dépend de #16 (Scaleway Storage). Le recadrage peut utiliser `cropperjs` ou équivalent.',
    }),
  },
  {
  ...
  },
  {
    ...
    body: userStoryBody({
      ...
      soThat: 'je dispose de ses coordonnées et informations professionnelles complètes',
      acceptanceCriteria: "- [ ] Page `/annuaire/[id]` accessible aux utilisateurs connectés\n- [ ] Affichage : photo, nom complet, fonction, badge département/service\n- [ ] Affichage : email professionnel, téléphone\n- [ ] Lien vers le profil du manager (si renseigné)\n- [ ] Bouton retour vers l'annuaire\n- [ ] Design conforme charte IUT-LCO",
      additionalContext: "Email et téléphone ne sont visibles que des utilisateurs authentifiés.",
    }),
  },
  {
    ...
    body: userStoryBody({
      asA: 'utilisateur connecté',
      iWant: 'filtrer et trier l\'annuaire par site, département, service ou rôle',
      soThat: "je retrouve rapidement les membres d'une équipe ou d'un site spécifique",
      acceptanceCriteria: "- [ ] Filtres : site (4 valeurs), département (8 + service transverse), rôle\n- [ ] Tri : nom alphabétique, département\n- [ ] Pagination server-side\n- [ ] Filtres mémorisés dans l'URL (query params)\n- [ ] Réutilise le pattern `DataTable` générique\n- [ ] Combinaison de filtres possible\n- [ ] Remise à zéro des filtres",
      additionalContext: "Étend l'Annuaire V1. Les filtres URL permettent de partager des liens vers des vues filtrées.",
    }),
  },
  {
    title: '[Feat] Modèle Article et tags',
    milestone: ms2,
    labels: ['type: feature', 'priority: critical', 'scope: news', 'scope: db'],
    body: userStoryBody({
      asA: 'développeur',
      iWant: 'avoir un modèle de données complet pour les articles et leurs tags',
      soThat: 'le système d\'actualités puisse gérer brouillons, publication immédiate, programmée et ciblage d\'audience',
      acceptanceCriteria: "- [ ] Modèle `Article` : titre, slug (unique), contenu riche (JSON TipTap), excerpt, coverUrl, authorId (FK User), status (enum DRAFT|PUBLISHED|SCHEDULED), publishedAt\n- [ ] Champ `audience` pour cibler départements/sites/spécialités\n- [ ] Modèle `Tag` : nom, slug\n- [ ] Relation many-to-many `Article ↔ Tag`\n- [ ] Migration et seed avec articles de test\n- [ ] Types partagés dans `packages/shared-types`",
      additionalContext: 'Le contenu riche est stocké au format JSON TipTap (ProseMirror). Le slug est généré automatiquement depuis le titre.',
    }),
  },
  {
    ...
    body: userStoryBody({
      ...
      soThat: "je puisse publier des actualités avec du contenu formaté, des images et un ciblage d'audience",
      acceptanceCriteria: "- [ ] Page `/admin/articles` : liste des articles avec statut (brouillon, publié, programmé)\n- [ ] Création d'article avec TipTap (titre, contenu riche, image de couverture)\n- [ ] Sélection de tags\n- [ ] Sélection d'audience (tous, par département, par site, par spécialité)\n- [ ] Publication immédiate ou programmée (date/heure)\n- [ ] Sauvegarde en brouillon\n- [ ] Prévisualisation avant publication\n- [ ] Accès réservé aux rôles `editor` et `admin` via `editorProcedure`",
      additionalContext: "Dépend du Modèle Article. L'upload d'images dans TipTap est traité séparément.",
    }),
  },
  {
    ...
    body: userStoryBody({
      asA: 'rédacteur',
      iWant: "insérer des images dans l'éditeur TipTap par glisser-déposer ou copier-coller",
      soThat: 'mes articles soient visuellement riches sans quitter l\'éditeur',
      acceptanceCriteria: "- [ ] Drag & drop d'image dans l'éditeur → upload automatique vers Scaleway\n- [ ] Coller une image depuis le presse-papiers → upload automatique\n- [ ] Barre d'outils avec bouton d'insertion d'image\n- [ ] Redimensionnement automatique (max-width configurable)\n- [ ] Indicateur de progression d'upload\n- [ ] Formats acceptés : JPEG, PNG, WebP, GIF\n- [ ] Taille max : 10 Mo",
  ...
  },
  {
    ...
    body: userStoryBody({
      asA: 'utilisateur connecté',
      iWant: 'consulter les actualités publiées, filtrées par tag ou département',
      soThat: 'je reste informé des nouvelles pertinentes pour mon département ou l\'ensemble de l\'IUT',
      acceptanceCriteria: "- [ ] Page `/actualites` accessible aux utilisateurs connectés\n- [ ] Grille d'articles publiés avec image de couverture, titre, excerpt, date, auteur\n- [ ] Pagination côté serveur\n- [ ] Filtres par tag\n- [ ] Filtres par département (avec code couleur)\n- [ ] Barre de recherche plein texte\n- [ ] Articles ciblés sur un département affichés avec son code couleur de spécialité",
      additionalContext: "Les articles avec audience spécifique ne s'affichent qu'aux utilisateurs concernés.",
    }),
  },
  {
  ...
  },
  {
    ...
    body: taskBody({
      taskType: '⚙️ Backend',
      description: "Implémenter les règles de permissions pour les articles : création/édition réservée aux admin et editor, lecture filtrée selon l'audience, brouillons visibles uniquement par l'auteur et les admins.",
      steps: "- [ ] `editorProcedure` : création, édition, suppression d'articles\n- [ ] `authenticatedProcedure` : lecture des articles publiés selon audience\n- [ ] Filtre audience : article ciblé sur un département = visible uniquement par les membres\n- [ ] Brouillons : visibles uniquement par auteur et admin\n- [ ] Articles SCHEDULED : non accessibles en lecture publique avant `publishedAt`\n- [ ] Tests unitaires des règles de permissions",
      additionalContext: "Un utilisateur en service transverse voit tous les articles sans restriction de département.",
    }),
  },
  {
    ...
    body: taskBody({
      taskType: '⚙️ Backend',
      description: "Mettre en place un job périodique qui passe automatiquement les articles `SCHEDULED` en `PUBLISHED` lorsque la date `publishedAt` est atteinte.",
      steps: "- [ ] Créer un job cron (ou équivalent) dans `packages/api`\n- [ ] Requête : `UPDATE Article SET status = PUBLISHED WHERE status = SCHEDULED AND publishedAt <= NOW()`\n- [ ] Gérer les erreurs et logger les publications effectuées\n- [ ] Configurer la fréquence (ex: toutes les minutes)\n- [ ] Tester avec un article programmé dans le passé",
      additionalContext: "Stocker les dates en UTC. Peut utiliser `node-cron` ou un système de queue.",
    }),
  },
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    body: taskBody({
      taskType: '⚙️ Backend',
      description: "Créer le package `email` avec templates Handlebars. Système de cache des templates. Helpers pour envoi via Resend. Templates aux couleurs IUT-LCO. Interface de prévisualisation email (route dev-only).",
      steps: "- [ ] Créer `packages/email` avec Resend SDK\n- [ ] Configurer variable `RESEND_API_KEY`\n- [ ] Créer le layout Handlebars : header avec logo IUT-LCO, footer avec coordonnées\n- [ ] Appliquer les couleurs IUT-LCO dans les templates\n- [ ] Système de cache des templates compilés\n- [ ] Helper `sendEmail(to, subject, template, data)`\n- [ ] Route dev-only `/dev/email-preview` pour visualiser les templates\n- [ ] Template de test (email de bienvenue)",
      additionalContext: "Les templates Handlebars doivent être responsives et compatibles avec les principaux clients email.",
    }),
  },
  {
    title: '[Feat] Modèle Subscription newsletter',
    milestone: ms3,
    labels: ['type: feature', 'priority: high', 'scope: newsletter', 'scope: db'],
    body: userStoryBody({
      asA: 'développeur',
      iWant: 'avoir un modèle de données pour les abonnements à la newsletter avec des tokens de désabonnement sécurisés',
      soThat: 'les utilisateurs puissent gérer leurs abonnements de manière sécurisée et conforme RGPD',
      acceptanceCriteria: "- [ ] Modèle `NewsletterSubscription` : userId (FK), frequency (enum WEEKLY|MONTHLY), isActive, unsubscribeToken, lastSentAt\n- [ ] Token de désabonnement : généré via `crypto.randomBytes`, single-use\n- [ ] Migration et seed\n- [ ] Types partagés dans `packages/shared-types`\n- [ ] Procédures tRPC : `subscribe`, `unsubscribe`, `updateFrequency`",
      additionalContext: "Les tokens sont single-use : une fois le désabonnement effectué, le token est invalidé. Conforme aux exigences RGPD.",
    }),
  },
  {
    ...
    body: userStoryBody({
      ...
      iWant: 'recevoir une newsletter périodique avec les articles publiés depuis le dernier envoi',
      soThat: "je reste informé sans avoir à me connecter à l'intranet tous les jours",
      acceptanceCriteria: "- [ ] Job périodique (hebdomadaire/mensuel selon fréquence abonné)\n- [ ] Génération d'un digest des articles publiés depuis `lastSentAt`\n- [ ] Template Handlebars conforme charte IUT-LCO\n- [ ] Envoi groupé via Resend (batch)\n- [ ] Lien de désabonnement dans chaque email\n- [ ] Mise à jour de `lastSentAt` après envoi\n- [ ] Gestion des erreurs d'envoi (retry, log)",
      additionalContext: "Les articles ciblés sur un département ne doivent être inclus que pour les abonnés du département concerné.",
    }),
  },
  {
    ...
    body: userStoryBody({
      asA: 'utilisateur',
      iWant: 'gérer mon abonnement à la newsletter et me désabonner via un lien email',
      soThat: 'je contrôle la fréquence de réception et puisse me désabonner facilement',
      acceptanceCriteria: "- [ ] Section newsletter dans la page `/profil` : toggle actif/inactif, sélection fréquence\n- [ ] Page publique de désabonnement `/newsletter/unsubscribe?token=xxx`\n- [ ] Token vérifié et invalidé après désabonnement (single-use)\n- [ ] Confirmation de désabonnement affichée\n- [ ] Lien de désabonnement dans chaque email de newsletter\n- [ ] Design conforme charte IUT-LCO",
      additionalContext: "La page de désabonnement est publique (accessible sans connexion). Le token doit être validé côté serveur.",
    }),
  },
  {
    title: '[Feat] Modèle Event et agenda',
    milestone: ms3,
    labels: ['type: feature', 'priority: critical', 'scope: agenda', 'scope: db'],
    body: userStoryBody({
      asA: 'développeur',
      iWant: 'avoir un modèle de données complet pour les événements et les participations',
      soThat: "l'agenda puisse gérer invitations, RSVP et exports iCal",
      acceptanceCriteria: "- [ ] Modèle `Event` : titre, description, startAt, endAt, location, site (enum Site), departmentId (FK optionnel), videoLink, organizerId (FK User), audience, isPublic\n- [ ] Modèle `EventParticipant` : eventId, userId, status (enum PENDING|ACCEPTED|DECLINED)\n- [ ] Migration et seed avec événements de test\n- [ ] Types partagés dans `packages/shared-types`",
      additionalContext: "Le champ `videoLink` est libre pour le moment (décision outil visio reportée à une réunion dédiée).",
    }),
  },
  {
    ...
    body: userStoryBody({
      asA: 'utilisateur connecté',
      iWant: "consulter l'agenda de l'IUT-LCO en vue mensuelle et hebdomadaire",
      soThat: 'je visualise facilement les événements à venir, filtrés par site ou département',
      acceptanceCriteria: "- [ ] Page `/agenda` accessible aux utilisateurs connectés\n- [ ] Vue mensuelle et vue hebdomadaire (toggle)\n- [ ] Navigation précédent/suivant/aujourd'hui\n- [ ] Filtres : site, département, audience\n- [ ] Événements colorés selon département/spécialité\n- [ ] Clic sur un événement → détail (titre, lieu, horaire, lien vidéo si renseigné)\n- [ ] Responsive mobile",
      additionalContext: "Peut utiliser `v-calendar` ou `FullCalendar`. Les couleurs suivent les tokens de spécialité.",
    }),
  },
  {
    ...
    body: userStoryBody({
      ...
      soThat: "mes collègues soient informés des réunions, formations et événements que j'organise",
      acceptanceCriteria: "- [ ] Formulaire de création : titre, description, date début/fin, lieu, site, département (optionnel), vidéo link, audience\n- [ ] Pré-remplissage : organisateur = utilisateur courant, site = site de l'utilisateur\n- [ ] Validation Zod côté serveur et client\n- [ ] Édition possible par organisateur et admin\n- [ ] Suppression avec confirmation\n- [ ] Champ `videoLink` libre (texte, pas de validation d'outil spécifique)",
      additionalContext: "La décision sur l'outil visio est reportée à une réunion dédiée.",
    }),
  },
  {
  ...
  },
  {
    ...
    body: userStoryBody({
      asA: 'utilisateur connecté',
      iWant: "exporter mon agenda IUT-LCO vers mon application de calendrier (Google Calendar, Outlook…)",
      soThat: "je n'aie pas à vérifier l'intranet pour connaître mes événements",
      acceptanceCriteria: "- [ ] Endpoint `/api/calendar/[token].ics` générant un flux iCal valide (RFC 5545)\n- [ ] Token personnalisé stable par utilisateur (non single-use, révocable)\n- [ ] Inclut : événements créés par l'utilisateur + événements où il est invité\n- [ ] Champs iCal : SUMMARY, DESCRIPTION, DTSTART, DTEND, LOCATION, URL\n- [ ] Page de configuration dans `/profil` : affichage URL, bouton de régénération du token\n- [ ] Content-Type `text/calendar`",
      additionalContext: "Utiliser la librairie `ical-generator` ou similaire.",
    }),
  },
  {
    ...
    body: userStoryBody({
      asA: 'utilisateur connecté',
      iWant: 'accéder à un espace RH listant les documents officiels par catégorie',
      soThat: 'je trouve facilement les formulaires, procédures et informations RH dont j\'ai besoin',
      acceptanceCriteria: "- [ ] Modèle `HRDocument` : titre, description, fileUrl, category, publishedAt, audience\n- [ ] Page `/rh` accessible aux utilisateurs connectés\n- [ ] Documents listés par catégorie avec titre, description, date\n- [ ] Téléchargement PDF via lien signé Scaleway\n- [ ] Filtres par catégorie\n- [ ] Migration et seed\n- [ ] Types partagés dans `packages/shared-types`",
      additionalContext: "L'audience suit la même logique que les articles. L'interface admin RH est traitée séparément.",
    }),
  },
  {
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    body: userStoryBody({
      asA: 'développeur',
      iWant: 'avoir un modèle de données pour les albums photo et les photos avec modération',
      soThat: "la photothèque permette l'upload par tous et la modération par les admins",
      acceptanceCriteria: "- [ ] Modèle `Album` : titre, description, coverUrl, site (enum Site), departmentId (optionnel FK), createdBy (FK User)\n- [ ] Modèle `Photo` : url, caption, albumId (FK), uploadedBy (FK User), status (enum PENDING|APPROVED|REJECTED), rejectionReason\n- [ ] Relation `Photo ↔ Tag` (many-to-many)\n- [ ] Migration et seed avec album de test\n- [ ] Types partagés dans `packages/shared-types`",
      additionalContext: "Les photos sont PENDING par défaut. Seules les photos APPROVED sont visibles dans la galerie publique.",
    }),
  },
  {
  ...
  },
  {
    ...
    body: userStoryBody({
      ...
      soThat: 'seules des photos appropriées soient visibles dans la galerie publique',
      acceptanceCriteria: "- [ ] Interface `/admin/photos` réservée au rôle `admin`\n- [ ] Liste des photos en attente (PENDING) avec aperçu, uploader, date\n- [ ] Actions : Approuver → status APPROVED, Rejeter → status REJECTED + motif\n- [ ] Email de notification à l'uploader en cas de rejet\n- [ ] Filtres par statut (PENDING, APPROVED, REJECTED)\n- [ ] Suppression physique possible par l'admin",
      additionalContext: "Dépend du package email (Resend). L'approbation rend la photo immédiatement visible dans la galerie.",
    }),
  },
  {
    title: '[Feat] Composant LazyLoad photos',
    milestone: ms4,
    labels: ['type: feature', 'priority: medium', 'scope: photos', 'scope: ui'],
    body: taskBody({
      taskType: '🎨 Frontend',
      description: "Créer ou adapter un composant de chargement progressif des images pour la galerie photo. IntersectionObserver pour le lazy load, états progressifs (placeholder, chargement, erreur).",
      steps: "- [ ] Composant `LazyImage.vue` avec IntersectionObserver\n- [ ] Placeholder (skeleton ou blur hash)\n- [ ] État de chargement progressif (fade-in)\n- [ ] État d'erreur avec fallback image\n- [ ] Intégration dans la galerie photos\n- [ ] Composant typé TypeScript",
      additionalContext: "Améliore les performances de la galerie. Réutilise les patterns existants si disponibles.",
    }),
  },
  {
    ...
    body: userStoryBody({
      ...
      soThat: 'je me repère facilement sur les 4 sites (Boulogne, Calais, Dunkerque, Saint-Omer)',
      acceptanceCriteria: "- [ ] Page `/plans` avec sélection du site (tabs ou dropdown)\n- [ ] Affichage du plan correspondant (image ou PDF)\n- [ ] Viewer avec zoom et déplacement (pinch-to-zoom mobile)\n- [ ] 4 sites : Boulogne-sur-Mer, Calais, Dunkerque, Saint-Omer\n- [ ] Plans hébergés sur Scaleway Object Storage\n- [ ] Téléchargement PDF possible\n- [ ] Design conforme charte IUT-LCO",
      additionalContext: "Les plans (images ou PDF) sont fournis par l'IUT. Si disponibles en PDF, utiliser un viewer PDF léger (ex: `pdfjs-dist`).",
    }),
  },
  {
  ...
  },
  {
    ...
    body: userStoryBody({
      asA: 'utilisateur connecté',
      iWant: "visualiser l'organigramme de l'IUT-LCO généré automatiquement depuis les données des utilisateurs",
      soThat: 'je comprenne la hiérarchie et puisse naviguer vers les fiches profil',
      acceptanceCriteria: "- [ ] Page `/organigramme` accessible aux utilisateurs connectés\n- [ ] Génération automatique depuis `User.managerId`\n- [ ] Visualisation arborescente groupée par site puis département\n- [ ] Nœuds cliquables → lien vers fiche profil\n- [ ] Couleurs par spécialité (tokens de la charte)\n- [ ] Responsive / zoom\n- [ ] Gestion des utilisateurs sans manager (racines de l'arbre)\n- [ ] Gestion des services transverses",
      additionalContext: "Peut utiliser `d3-hierarchy`, `vue-org-tree` ou un composant custom.",
    }),
  },
  {
  ...
  },
  {
    ...
    body: taskBody({
      taskType: '🔍 Research / Spike',
      description: "Feature bonus à définir selon les priorités des commanditaires en fin de Sprint 8. Candidats : visio intégrée, recherche globale, notifications in-app, export annuaire.",
      steps: "- [ ] Recueillir les retours commanditaires en fin de Sprint 8\n- [ ] Voter/prioriser la feature bonus\n- [ ] Cadrage rapide (user story, critères d'acceptation)\n- [ ] Mettre à jour cette issue avec le scope définitif\n- [ ] Implémentation",
      additionalContext: "Candidats : visio intégrée (réunion dédiée), recherche globale (Meilisearch ?), notifications in-app, export annuaire (CSV/Excel).",
    }),
  },
  {
  ...
  },
  {
    ...
    body: taskBody({
      taskType: '⚙️ Backend',
      description: "Reprise du code livré pendant les 4 sprints : amélioration des patterns, extraction de la logique commune, nettoyage.",
      steps: "- [ ] Review du code des 4 sprints\n- [ ] Extraction de helpers/composables communs\n- [ ] Élimination des duplications\n- [ ] Vérification de la couverture TypeScript (strict mode)\n- [ ] Nettoyage des `TODO` et `FIXME`\n- [ ] Mise à jour de la documentation inline\n- [ ] Vérification des patterns tRPC (procédures, validations Zod)",
      additionalContext: "Basé sur les retours des reviews continues. Prioriser auth et permissions.",
    }),
  },
  {
...
for (const issue of issues) {
  if (dryRun) {
    console.log(`[DRY RUN] Would create: ${issue.title} → milestone ${issue.milestone}`);
    created++;
    continue;
  }
