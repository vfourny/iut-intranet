#!/usr/bin/env node
/**
 * Script de création des issues GitHub pour le projet IUT-LCO Intranet.
 * Utilise l'API GitHub REST. Les issues suivent les templates de @vfourny/.github.
 *
 * Usage :
 *   GITHUB_TOKEN=ghp_xxx node scripts/create-issues.mjs
 *
 * Prérequis :
 *   - Node.js 18+ (fetch natif)
 *   - Un token GitHub avec scope "repo" (issues:write)
 */

const OWNER = "vfourny";
const REPO = "iut-intranet";
const TOKEN = process.env.GITHUB_TOKEN;

if (!TOKEN) {
  console.error("❌ GITHUB_TOKEN manquant. Exporte-le avant de lancer le script.");
  process.exit(1);
}
...
async function getMilestones() {
  const data = await apiGet("/milestones?state=all&per_page=50");
  const map = {};
  for (const m of data) {
    map[m.title] = m.number;
  }
  console.log("📌 Milestones trouvés :", JSON.stringify(map, null, 2));
  return map;
}
  ...
  return [
    ...
    },
    {
      ...
      body: taskBody({
        taskType: "⚙️ Backend",
        description:
          "Initialiser Prisma dans `packages/db`. Configurer la connexion PostgreSQL. Schéma initial avec `User`, `Session`, et les enums `Site`, `Department`, `Specialty`. Mettre en place le système de migrations et un seed TypeScript.",
        steps:
    ...
    },
    {
      ...
      body: taskBody({
        ...
        description:
          "Demander et récupérer auprès de l'IUT : fichiers logo principal (couleur, monochrome) en SVG, les 8 carrés de département en SVG, valeurs RGB officielles si différentes des conversions CMYK. Intégrer dans `packages/ui/assets/`.",
        steps:
          "- [ ] Contacter le service communication IUT-LCO\n- [ ] Obtenir logo principal couleur SVG\n- [ ] Obtenir logo principal monochrome SVG\n- [ ] Obtenir les 8 logos de département SVG\n- [ ] Vérifier les valeurs RGB officielles vs conversions CMYK du cahier des charges\n- [ ] Intégrer les assets dans `packages/ui/assets/`\n- [ ] Documenter les règles d'usage (zone de protection, interdits)",
        additionalContext:
          "Couleurs de référence du cahier des charges :\n- Rouge IUT : #E30613\n- Bleu IUT : #00AEEF\n- Gris IUT : #737373\n- Spécialités : voir plan projet",
      }),
    },
    {
      ...
      body: taskBody({
        taskType: "🎨 Frontend",
        description:
          "Définir tous les tokens CSS dans `packages/ui` : couleurs principales (rouge, bleu, gris IUT), couleurs des 5 spécialités, typographie Arial, espacements. Configurer Tailwind avec ces tokens. Documenter les règles d'usage du logo.",
        steps:
    ...
    },
    {
      ...
      body: taskBody({
        taskType: "🎨 Frontend",
        description:
          "Configurer Nuxt avec TypeScript, Tailwind, PrimeVue. Appliquer le thème PrimeVue conforme à la charte IUT-LCO (couleurs primaires, typo Arial). Mettre en place le layout de base.",
        steps:
    ...
    },
    {
      ...
      body: taskBody({
        taskType: "⚙️ Backend",
        description:
          "Configurer le serveur tRPC avec Inversify pour la DI. Intégrer Better Auth (email/password). Créer les middlewares de procédure : `publicProcedure`, `authenticatedProcedure`, `adminProcedure`, `editorProcedure`, `hrProcedure`. Mettre en place le client tRPC côté Nuxt avec Pinia Colada.",
        steps:
        ...
        additionalContext:
          "Les erreurs métier utilisent `AppError` custom propagées via Inversify. Toutes les procédures doivent être validées avec Zod.",
      }),
    },
    {
      ...
      body: userStoryBody({
        ...
        acceptanceCriteria:
          "- [ ] Enum `Site` : `BOULOGNE`, `CALAIS`, `DUNKERQUE`, `SAINT_OMER`\n- [ ] Enum `Specialty` : 5 valeurs (Administration/Gestion/Commerce, Électricité/Automatique/Informatique, Sciences Industrielles, Chimie/Biologie, Construction/Énergie)\n- [ ] Enum `Department` : 8 valeurs (GACO, GEA, TC, INFO, GEII, GIM, GB, GTE)\n- [ ] Table de référence `Department` seedée avec métadonnées (site, spécialité, couleur hex, libellé complet)\n- [ ] Types partagés dans `packages/shared-types` (consommable côté frontend)\n- [ ] Migration et seed fonctionnels",
        additionalContext:
          "Couleurs spécialités :\n- Administration/Gestion/Commerce (GACO, GEA, TC) : #D71F69\n- Électricité/Automatique/Informatique (INFO, GEII) : #0089CF\n- Sciences Industrielles (GIM) : #4D5C66\n- Chimie/Biologie (GB) : #7FB539\n- Construction/Énergie (GTE) : #EE7F00",
      }),
    },
    {
      ...
      body: userStoryBody({
        ...
        acceptanceCriteria:
          "- [ ] Champs : `firstName`, `lastName`, `email`, `phone`, `photoUrl`, `jobTitle`\n- [ ] Champ `departmentId` (FK optionnel vers `Department`)\n- [ ] Champ `service` (texte libre, pour le personnel transverse)\n- [ ] Champ `managerId` (auto-référence optionnelle)\n- [ ] Enum `Role` : `ADMIN`, `EDITOR`, `HR`, `USER`\n- [ ] Contrainte : soit `departmentId` soit `service` renseigné (pas les deux null)\n- [ ] Migration et seed avec quelques utilisateurs de test",
        additionalContext:
          "Les utilisateurs sans département rattaché (RH, scolarité, direction…) utilisent le champ `service` libre. Le champ `managerId` est nécessaire pour l'organigramme du Sprint 4.",
      }),
    },
    {
      ...
      body: userStoryBody({
        ...
        soThat:
          "j'accède à l'intranet de manière sécurisée",
        acceptanceCriteria:
        ...
        additionalContext:
          "Utilise Better Auth (configuré dans #6). Les tokens de reset doivent être cryptographiquement sécurisés et single-use.",
      }),
    },
    {
      ...
      body: userStoryBody({
        ...
        soThat:
          "je m'y repère facilement sur tous les appareils",
        acceptanceCriteria:
    ...
    },
    {
      ...
      body: userStoryBody({
        asA: "utilisateur connecté",
        iWant:
          "consulter la liste de mes collègues et les rechercher par nom",
        soThat:
          "je puisse retrouver rapidement les coordonnées d'un collègue",
        acceptanceCriteria:
        ...
        additionalContext:
          "Annuaire V2 avec filtres avancés prévu au Sprint 2 (issue #20). Cette V1 est le minimum viable pour la démo Sprint 1.",
      }),
    },
    {
      ...
      body: taskBody({
        taskType: "🛠️ DevOps / Infra",
        description:
          "Configurer GitHub Actions : lint, typecheck, build sur PR. Déploiement preview automatique. Coordination avec JRCanDev pour l'environnement de production.",
        steps:
          "- [ ] Workflow CI : lint + typecheck sur chaque PR\n- [ ] Workflow CI : build sur chaque PR\n- [ ] Workflow CD : déploiement preview automatique sur PR\n- [ ] Badge CI dans le README\n- [ ] Coordination avec JRCanDev pour l'env prod (voir #14)",
        additionalContext:
    ...
    },
    {
      ...
      body: taskBody({
        taskType: "🔍 Research / Spike",
        description:
          "Recueillir les contraintes d'hébergement auprès de JRCanDev (équipe interne IUT) : Docker ou non ? reverse proxy ? PostgreSQL provisionné ou à provisionner ? domaine ? certificats SSL ? procédure de déploiement ? Documenter dans le repo.",
        steps:
          "- [ ] Réunion avec l'équipe JRCanDev\n- [ ] Clarifier : Docker / docker-compose ou autre ?\n- [ ] Clarifier : reverse proxy (Nginx, Caddy, Traefik ?)\n- [ ] Clarifier : PostgreSQL provisionné ou à déployer ?\n- [ ] Clarifier : domaine et sous-domaine (ex: intranet.iut-littoral.fr)\n- [ ] Clarifier : certificats SSL (Let's Encrypt, wildcard ?)\n- [ ] Documenter la procédure de déploiement dans `docs/deployment.md`\n- [ ] Ouvrir les issues de suivi si des actions JRCanDev sont nécessaires",
        additionalContext:
          "Cette issue bloque potentiellement le déploiement preview (#13) et la mise en production finale (#52).",
      }),
    },
    {
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
      ...
      body: taskBody({
        taskType: "🛠️ DevOps / Infra",
        description:
          "Créer un bucket Scaleway Object Storage en région Paris. Créer le package `storage` dans le monorepo avec client S3-compatible. Helpers pour upload, génération d'URLs signées, suppression. Variables d'environnement.",
        steps:
          "- [ ] Créer le bucket Scaleway (région Paris, hébergement France)\n- [ ] Créer `packages/storage` avec client S3-compatible (`@aws-sdk/client-s3`)\n- [ ] Helper `uploadFile(key, buffer, mimeType)` → URL publique\n- [ ] Helper `getSignedUrl(key, expiresIn)` → URL signée\n- [ ] Helper `deleteFile(key)`\n- [ ] Configurer les variables d'env : `SCALEWAY_ACCESS_KEY`, `SCALEWAY_SECRET_KEY`, `SCALEWAY_BUCKET`, `SCALEWAY_ENDPOINT`\n- [ ] Mettre à jour `.env.example`\n- [ ] Tester upload d'une image de test",
        additionalContext:
          "Scaleway Object Storage est S3-compatible. Endpoint : `s3.fr-par.scw.cloud`. Utilisé pour photos de profil, images d'articles, documents RH et photos.",
      }),
    },
    {
      ...
      body: userStoryBody({
        asA: "utilisateur connecté",
        iWant:
          "uploader et recadrer une photo de profil",
        soThat:
    ...
    },
    {
      ...
      body: userStoryBody({
        asA: "utilisateur connecté",
        iWant:
          "consulter la fiche profil publique d'un collègue",
        soThat:
        ...
        additionalContext:
          "Les informations affichées respectent la politique de confidentialité interne. Email et téléphone ne sont visibles que des utilisateurs authentifiés.",
      }),
    },
    {
      ...
      body: userStoryBody({
        ...
        additionalContext:
          "Étend l'Annuaire V1 (issue #12). Les filtres URL permettent de partager des liens vers des vues filtrées.",
      }),
    },
    {
    ...
    },
    {
      ...
      body: userStoryBody({
        ...
        iWant:
          "créer et éditer des articles avec un éditeur riche TipTap",
        soThat:
        ...
        additionalContext:
          "Dépend de #21 (Modèle Article). L'upload d'images dans TipTap est traité séparément dans #23.",
      }),
    },
    {
      ...
      body: userStoryBody({
        ...
        additionalContext:
          "Dépend de #16 (Scaleway Storage) et #22 (Éditeur TipTap). Extension TipTap custom ou `@tiptap/extension-image` étendu.",
      }),
    },
    {
      ...
      body: userStoryBody({
        ...
        additionalContext:
          "Les articles avec audience spécifique ne s'affichent qu'aux utilisateurs concernés (voir #26 Permissions articles).",
      }),
    },
    {
      ...
      body: userStoryBody({
        asA: "utilisateur connecté",
        iWant:
          "lire le contenu complet d'un article",
        soThat:
          "je puisse me tenir informé de l'actualité de l'IUT",
        acceptanceCriteria:
        ...
        additionalContext:
          "Dépend de #24 (Liste actualités). Le rendu TipTap utilise `@tiptap/vue-3` en mode lecture seule.",
      }),
    },
    {
    ...
    },
    {
      ...
      body: taskBody({
        taskType: "⚙️ Backend",
        description:
          "Mettre en place un job périodique qui passe automatiquement les articles en statut `SCHEDULED` au statut `PUBLISHED` lorsque la date `publishedAt` est atteinte.",
        steps:
        ...
        additionalContext:
          "Peut utiliser `node-cron`, un job Turborepo, ou un système de queue si disponible. Attention aux fuseaux horaires (stocker en UTC).",
      }),
    },
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
      ...
      body: taskBody({
        taskType: "⚙️ Backend",
        description:
          "Créer le package `email` avec templates Handlebars. Système de cache des templates. Helpers pour envoi via Resend. Templates aux couleurs IUT-LCO (header avec logo, footer avec coordonnées). Interface de prévisualisation email (route Vue dev-only).",
        steps:
          "- [ ] Créer `packages/email` avec Resend SDK\n- [ ] Configurer variable `RESEND_API_KEY` dans `.env`\n- [ ] Créer le layout de base Handlebars : header avec logo IUT-LCO, footer avec coordonnées\n- [ ] Appliquer les couleurs IUT-LCO dans les templates\n- [ ] Système de cache des templates compilés\n- [ ] Helper `sendEmail(to, subject, template, data)`\n- [ ] Route dev-only `/dev/email-preview` pour visualiser les templates dans le navigateur\n- [ ] Template de test (email de bienvenue)",
        additionalContext:
          "Resend est le fournisseur email. Les templates Handlebars doivent être responsives et compatibles avec les principaux clients email.",
      }),
    },
    {
    ...
    },
    {
      ...
      body: userStoryBody({
        ...
        additionalContext:
          "La page de désabonnement est publique (accessible sans connexion) car le lien est dans l'email. Le token doit être validé côté serveur.",
      }),
    },
    {
      ...
      body: userStoryBody({
        ...
        soThat:
          "l'agenda puisse gérer invitations, RSVP et exports iCal",
        acceptanceCriteria:
          "- [ ] Modèle `Event` : titre, description, startAt, endAt, location, site (enum Site), departmentId (FK optionnel), videoLink, organizerId (FK User), audience, isPublic\n- [ ] Modèle `EventParticipant` : eventId, userId, status (enum PENDING|ACCEPTED|DECLINED)\n- [ ] Migration et seed avec quelques événements de test\n- [ ] Types partagés dans `packages/shared-types`",
        additionalContext:
          "Le champ `videoLink` est libre pour le moment (en attente de décision sur l'outil visio). Le champ `audience` suit la même logique que les articles.",
      }),
    },
    {
    ...
    },
    {
      ...
      body: userStoryBody({
        ...
        iWant:
          "inviter des participants et recevoir leurs réponses par email",
        soThat:
          "je sache combien de personnes participeront à mon événement",
        acceptanceCriteria:
        ...
        additionalContext:
          "Dépend de #28 (Resend) et #32 (Modèle Event). Les tokens RSVP suivent le même pattern que les tokens de désabonnement.",
      }),
    },
    {
      ...
      body: userStoryBody({
        ...
        additionalContext:
          "Dépend de #32 (Modèle Event). Utiliser la librairie `ical-generator` ou similaire.",
      }),
    },
    {
      ...
      body: userStoryBody({
        ...
        acceptanceCriteria:
          "- [ ] Modèle `HRDocument` : titre, description, fileUrl, category (ex: Congés, Formation, Déplacements…), publishedAt, audience\n- [ ] Page `/rh` accessible aux utilisateurs connectés\n- [ ] Documents listés par catégorie avec titre, description, date\n- [ ] Téléchargement PDF via lien signé Scaleway\n- [ ] Filtres par catégorie\n- [ ] Migration et seed\n- [ ] Types partagés dans `packages/shared-types`",
        additionalContext:
          "L'audience suit la même logique que les articles. L'interface admin RH est traitée dans #38.",
      }),
    },
    {
      ...
      body: userStoryBody({
        ...
        iWant:
          "uploader et gérer les documents RH depuis une interface dédiée",
        soThat:
          "les documents soient disponibles et à jour pour le personnel",
        acceptanceCriteria:
        ...
        additionalContext:
          "Dépend de #16 (Scaleway) et #37 (Modèle HRDocument). Accès strictement limité aux rôles `hr` et `admin`.",
      }),
    },
    ...
    },
    {
      ...
      body: userStoryBody({
        ...
        soThat:
          "je partage des moments de la vie de l'IUT avec mes collègues",
        acceptanceCriteria:
        ...
        additionalContext:
          "Dépend de #16 (Scaleway) et #39 (Modèle Photo/Album). La modération est dans #41.",
      }),
    },
    {
      ...
      body: userStoryBody({
        asA: "administrateur",
        iWant:
          "modérer les photos uploadées par les utilisateurs",
        soThat:
        ...
        additionalContext:
          "Dépend de #28 (Resend) pour les notifications. L'approbation rend la photo immédiatement visible dans la galerie.",
      }),
    },
    {
      ...
      body: taskBody({
        taskType: "🎨 Frontend",
        description:
          "Créer ou adapter un composant de chargement progressif des images pour la galerie photo. Utiliser IntersectionObserver pour le lazy load et gérer les états progressifs (placeholder, chargement, erreur).",
        steps:
          "- [ ] Composant `LazyImage.vue` avec IntersectionObserver\n- [ ] Placeholder (skeleton ou blur hash)\n- [ ] État de chargement progressif (fade-in)\n- [ ] État d'erreur avec fallback image\n- [ ] Intégration dans la galerie photos (`/photos` et `/photos/[albumId]`)\n- [ ] Composant typé TypeScript\n- [ ] Tests visuels sur mobile et desktop",
        additionalContext:
          "Réutilise les patterns LazyLoad + ImageDisplay du projet si déjà existants. Améliore les performances de la galerie.",
      }),
    },
    {
      ...
      body: userStoryBody({
        asA: "utilisateur connecté",
        iWant:
          "consulter le plan d'un site de l'IUT-LCO",
        soThat:
    ...
    },
    {
      ...
      body: userStoryBody({
        ...
        soThat:
          "je sache comment préparer et déclarer mes déplacements",
        acceptanceCriteria:
        ...
        additionalContext:
          "Le modèle `InfoPage` est réutilisable pour d'autres pages de contenu statique (mentions légales, etc.).",
      }),
    },
    {
    ...
    },
    {
      ...
      body: docBody({
        ...
        additionalContext:
          "Les guides doivent être illustrés de captures d'écran. La page `/aide` liste les guides disponibles avec liens de téléchargement.",
      }),
    },
    {
      ...
      body: docBody({
        ...
        additionalContext:
          "À compléter progressivement pendant les 4 sprints. La documentation de déploiement est critique pour la mise en prod (Sprint 5).",
      }),
    },
    ...
    // ────────────────────────────────────────────────────────────────────────
    {
      ...
      milestone: ms5,
      labels: ["type: feature", "priority: medium", "scope: auth", "security"],
      body: userStoryBody({
        asA: "utilisateur IUT-LCO",
        iWant:
          "me connecter à l'intranet avec mon compte UlCO (SSO)",
        soThat:
          "je n'aie pas à gérer un mot de passe supplémentaire",
        acceptanceCriteria:
        ...
        additionalContext:
          "⚠️ Conditionnel à la réception d'une réponse de l'UlCO. Si pas de réponse, cette issue reste en backlog. Bloque potentiellement sur status: blocked.",
      }),
    },
    {
      title: "[Feat] Feature bonus Sprint 9",
      milestone: ms5,
      ...
      body: taskBody({
        ...
        steps:
          "- [ ] Recueillir les retours commanditaires en fin de Sprint 8\n- [ ] Voter/prioriser la feature bonus\n- [ ] Cadrage rapide (user story, critères d'acceptation)\n- [ ] Mise à jour de cette issue avec le scope définitif\n- [ ] Implémentation",
        additionalContext:
          "Remplacer le titre de cette issue par le nom de la feature réelle une fois définie. Candidats :\n- Visio intégrée (à cadrer après réunion dédiée)\n- Recherche globale (Meilisearch ?)\n- Notifications in-app\n- Export annuaire (CSV/Excel)",
      }),
    },
    {
    ...
    },
    {
      ...
      body: taskBody({
        taskType: "⚙️ Backend",
        description:
          "Reprise du code livré pendant les 4 sprints : amélioration des patterns, extraction de la logique commune, nettoyage et application des conventions du projet.",
        steps:
        ...
        additionalContext:
          "Basé sur les retours des reviews continues pendant les 8 semaines de stage. Prioriser les sections les plus critiques (auth, permissions).",
      }),
    },
    {
      ...
      body: taskBody({
        ...
        additionalContext:
          "Dépend de #14 (Coordination JRCanDev) et #51 (Hardening sécurité). La mise en production nécessite que tous les tests E2E (#50) passent.",
      }),
    },
  ];
}
...
async function main() {
  console.log("🚀 Démarrage de la création des issues IUT-LCO Intranet...\n");

  // 1. Récupérer les milestones
  const milestones = await getMilestones();

  // 2. Construire la liste des issues
  const issues = buildIssues(milestones);

  console.log(`\n📋 ${issues.length} issues à créer...\n`);
  ...
  for (const issue of issues) {
    try {
      ...
      if (issue.milestone) {
        ...
        console.warn(`  ⚠️  Milestone non trouvé pour : ${issue.title}`);
      }

      const result = await apiPost("/issues", payload);
      console.log(`  ✅ #${result.number} — ${issue.title}`);
      results.push({ number: result.number, title: issue.title, url: result.html_url });
      created++;

      // Rate limiting : pause de 500ms entre chaque issue
      await sleep(500);
    } catch (err) {
